...
export default function VesselDetailsBox() {
    ...
    useEffect(() => {
        const fetchDetails = async () => {
            ...
            const details = await clientHandler.getVesselInfo({ mmsi: selectedVesselmmsi, timestamp: myDateTime.getTime() })
            setVesselDetails(details)
        }

        fetchDetails()
        setLoading(false)
    }, [clientHandler, myDateTime, selectedVesselmmsi])

    async function getVesselPath() {
        if (!selectedVesselmmsi) return
        const res = await clientHandler.getVesselPath({ mmsi: selectedVesselmmsi, endtime: myDateTime.getTime() / 1000, starttime: myDateTime.getTime() / 1000 - (pathDuration * 60 * 60) }) //time is in seconds
            ...
            {loading ? (
                ...
                vesselDetails && (
                    <div>
                        ...
                        <button title="Close vessel details" className="absolute top-4 right-4" onClick={() => setSelectedVesselmmsi(undefined)}>
                            <CloseSVG />
                        </button>
                        {!isCollapsed && (
                            <>
                                <div id="vessel-info" className="flex flex-col">
                                    {
                                        vesselDetailsContent.map((content, index) => (
                                            <div className="flex flex-row gap-3 py-1 px-2 rounded-md even:bg-gray-700 odd:bg-gray-600" key={index}>
                                                <p className="w-20 font-semibold">{content.displayName}</p>
                                                <p title={content.value?.toString()} className="truncate">{content.value}</p>
                                            </div>
                                        ))
                                    }
                                </div>
                                ...
                                <div className="flex flex-row justify-between items-center gap-2">
                                    <div className="flex flex-row gap-2">
                                        ...
                                        <input className="w-12 rounded-md text-center bg-gray-600" type="number" placeholder="Path duration  Hours" value={pathDuration} onChange={(e) => handleDurationChange(e.target.value)}></input>
                                    </div>
                                    <button className="blue-btn" onClick={getVesselPath}>Show Path</button>
                                </div>
                            </>
                        )}
                        <button title="Hide/show vessel details" onClick={() => setIsCollapsed(!isCollapsed)} className="w-full flex flex-cols items-center justify-center bottom-0">
